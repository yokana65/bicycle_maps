...
```{r}
biketr <- function (route = all_stops_unique, option = "bike", write = FALSE) {
...
```{r}
traveling_map <- function(municipalities_sf, all_stops_unique, routes_geocoded) {
  ...
  ggsave(filename = "route_overview.png", plot = p, width = 6, height = 4, dpi = 300)

  return(p)
}

traveling_map_daily <- function(municipalities_sf, all_stops_unique, routes_geocoded, day = 1) {
  theme_roadtrip <- function() {
    theme_void(base_family = "Overpass Light") +
    theme(
      plot.title = element_text(family = "Overpass", face = "bold", hjust = 0.5),
